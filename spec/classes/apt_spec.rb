...
describe 'apt' do
  ...
  describe 'Test standard installation' do
    ...
    it { should contain_service('apt').with_ensure('running') }
    it { should contain_service('apt').with_enable('true') }
  ...
  describe 'Test standard installation with monitoring and firewalling' do
    ...
    it 'should monitor the process' do
      content = catalogue.resource('monitor::process', 'apt_process').send(:parameters)[:enable]
      content.should == true
    end
    it 'should place a firewall rule' do
      content = catalogue.resource('firewall', 'apt_tcp_42').send(:parameters)[:enable]
      content.should == true
    end
  end
  ...
  describe 'Test decommissioning - absent' do
    ...
    it 'should stop Service[apt]' do should contain_service('apt').with_ensure('stopped') end
    it 'should not enable at boot Service[apt]' do should contain_service('apt').with_enable('false') end
  ...
  describe 'Test decommissioning - disableboot' do
    ...
    it { should contain_package('apt').with_ensure('present') }
    it { should_not contain_service('apt').with_ensure('present') }
    it { should_not contain_service('apt').with_ensure('absent') }
    it 'should not enable at boot Service[apt]' do should contain_service('apt').with_enable('false') end
    it { should contain_file('apt.conf').with_ensure('present') }
  ...
  describe 'Test service autorestart', :broken => true do
    it 'should automatically restart the service, by default' do
      content = catalogue.resource('file', 'apt.conf').send(:parameters)[:notify]
      content.should == 'Service[apt]{:name=>"apt"}'
    end
  end
  ...
  describe 'Test Monitoring Tools Integration' do
    ...
    it 'should generate monitor defines' do
      content = catalogue.resource('monitor::process', 'apt_process').send(:parameters)[:tool]
  ...
  describe 'Test Firewall Tools Integration' do
    ...
    it 'should generate correct firewall define' do
      content = catalogue.resource('firewall', 'apt_tcp_42').send(:parameters)[:tool]
  ...
  describe 'Test OldGen Module Set Integration' do
    ...
    it 'should generate puppi resources ' do 
      content = catalogue.resource('puppi::ze', 'apt').send(:parameters)[:ensure]
  ...
  describe 'Test params lookup' do
    let(:facts) { { :apt_monitor => true , :ipaddress => '10.42.42.42' } }
    let(:params) { { :port => '42' } }
  ...
  describe 'Test params lookup' do
    let(:facts) { { :monitor => false , :apt_monitor => true , :ipaddress => '10.42.42.42' } }
    let(:params) { { :port => '42' } }
